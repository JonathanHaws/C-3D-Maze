...
struct Maze {
    ...
    Mesh wall = Mesh("meshes/point.obj"); 
    ...
    Maze(int width, int depth, float height, float breadth, float scale, float speed, Camera& camera, bool loop = false, Shader* shader2d = nullptr, Shader* mazeShader = nullptr) :
        width(width), depth(depth), height(height), breadth(breadth), scale(scale), speed(speed), camera(camera), loop(loop), shader2d(shader2d), mazeShader(mazeShader) {
    ...
    void update_texture(int xoffset = 0, int yoffset = 0, int width = -1, int depth = -1) {
    ...
    void expand() {
        if (corridors.size() != width || corridors[0].size() != depth) { // Avoid accessing out of bounds for stack and corridors
            mazeTexture.resize(width, depth);
            reset();
        }

        if (cells_to_expand.empty()) {
            if (getValue(1, 1) == ' ') { // Maze is complete
                if (loop) { reset(); }
                return;
            }
        ...
        for (const auto& dir : directions) {
            ...
            if (nx >= 0 && ny >= 0 && nx < width && ny < depth) {
                if (getValue(cell.first + dir.dx, cell.second + dir.dy) == '#' && getValue(nx, ny) == '#') {
                    corridors[cell.first + dir.dx][cell.second + dir.dy] = ' ';
                    update_texture(cell.first + dir.dx, cell.second + dir.dy, 1, 1);
                    corridors[nx][ny] = ' ';
                    update_texture(nx, ny, 1, 1);
        ...
        update_texture(cell.first, cell.second, 1, 1);
        cells_to_expand.pop();
    }
